...
| F3 | Esqueleto Calamares (happy path T1) | 🚧 en progreso (código completo; falta build + test T1) | Paquete `pkg/calamares-config-amicachy/` ✅, `build_target_rootfs.sh` ✅, flip `amilaunch.sh` ✅, integración `build_iso.sh` ✅. Pendiente: construir `target.sfs` + ISO y validar T1 en VM (requiere Docker/sudo) |
