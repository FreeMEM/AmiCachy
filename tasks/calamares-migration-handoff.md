...
| F3 | Esqueleto Calamares (happy path T1) | 🚧 build OK; falta validar T1 (test gráfico interactivo) | Código ✅. `target.sfs` construido y verificado (2.1 GB, HOOKS plymouth, Theme=amicachy, 0 .pacnew, sin instalador dentro). ISO `out/amicachy-v3-2026.06.13-x86_64.iso` (4.4 GB) construida OK. `build_target_rootfs.sh` necesitó 5 fixes (repo-add rust, overwrite, .pacnew — ver commit 8a8cea2). Pendiente: arrancar la ISO y completar el wizard Calamares en la VM (interactivo) |
