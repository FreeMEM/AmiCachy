...
| F3 | Esqueleto Calamares (happy path T1) | 🚧 1er test T1 hecho: cadena OK, faltaba shim de libs (arreglado); re-test pendiente | Toda la cadena funciona (Plymouth→autologin→amilaunch→cage→sudo→calamares lanza). 1er T1 falló SOLO por `cachyos-calamares 3.4.1-8` **stale** (compilado vs yaml-cpp 0.8 / boost 1.89; repo tiene 0.9 / 1.91 → `libyaml-cpp.so.0.8`/`libboost_*.so.1.89.0 not found`). Arreglado con `pkg/calamares-compat-libs` (shim temporal, libs viejas del Arch Archive; commit af5524f). **Re-test**: `sudo build_iso.sh --cpu-arch v3` (reusa el target.sfs) → boot-iso. `target.sfs` y 5 fixes de `build_target_rootfs.sh` en 8a8cea2 |
