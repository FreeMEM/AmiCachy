...
# --- Early Startup Control (hold F5 during the Plymouth splash) ---
# The Plymouth theme tells the user to hold F5; check_hotkey takes a
# snapshot of the keyboard state when it runs, so the key has to still
# be pressed at that moment. The window inside check_hotkey is 8 s and
# tunable via $AMICACHY_HOTKEY_WINDOW.
EARLY_STARTUP_UAE=""
