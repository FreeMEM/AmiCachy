...
# ---------------------------------------------------------------------------
# FS-UAE template rendering
#
# FS-UAE configs share *some* concepts with .uae but use entirely different
# key names (chip_memory in KB instead of chipmem_size with Amiberry-specific
# units, hard_drive_N instead of hardfile2/uaehf0, etc.). We don't try to
# auto-translate from a uae_template — catalog entries declare a separate
# `fsuae_template` dict whose keys we serialize verbatim.
# ---------------------------------------------------------------------------
...
def render_fsuae_template_fs(
    template: dict,
    fs_root: Path,
    extract_root: Path,
    volume_name: str = "System",
) -> str:
    """Generate a .fs-uae config that mounts a directory as DH0.

    `template` follows FS-UAE syntax (e.g. `chip_memory = 8192`,
    `cpu = 68020`, `graphics_card = uaegfx-z3`); placeholder values like
    ${FS_ROOT} get substituted before serialization.
    """
    ctx = {
        "EXTRACT_ROOT": str(extract_root),
        "FS_ROOT": str(fs_root),
    ...
    for key, value in template.items():
        if isinstance(value, bool):
            ...
            value = _substitute(value, ctx)
        lines.append(f"{key} = {value}")
    # Mount the directory tree as DH0 with high boot priority.
    lines.append(f"hard_drive_0 = {fs_root}")
    lines.append(f"hard_drive_0_label = {volume_name}")
    lines.append(f"hard_drive_0_priority = 127")
    lines.extend(_FSUAE_BOILERPLATE)
    return "\n".join(lines) + "\n"


def render_fsuae_template_hdf(
    template: dict,
    hdf_path: Path,
    extract_root: Path,
) -> str:
    """Generate a .fs-uae config that mounts a hardfile as DH0."""
    ctx = {"EXTRACT_ROOT": str(extract_root), "HDF": str(hdf_path)}
    lines = ["# AmiCachy — auto-generated by fetch_asset"]
    for key, value in template.items():
        if isinstance(value, bool):
            value = "1" if value else "0"
            ...
            value = _substitute(value, ctx)
        lines.append(f"{key} = {value}")
    lines.append(f"hard_drive_0 = {hdf_path}")
    lines.append("hard_drive_0_label = DH0")
    lines.append("hard_drive_0_priority = 127")
    lines.extend(_FSUAE_BOILERPLATE)
    return "\n".join(lines) + "\n"


def register_uae(asset: Asset, extracted_root: Path) -> Path | None:
    """Generate ~/Amiberry/conf/amicachy-<id>.{uae,fs-uae} for an asset.

    Always produces a `.uae` (Amiberry config) when the install spec has
    `uae_template`. Additionally produces a `.fs-uae` companion when the
    spec has `fsuae_template` — both files live in the same directory so
    the Early Startup Control lists them side by side, tagged with the
    emulator they target. The user picks one at boot time and amilaunch
    dispatches accordingly.

    Branches on install.layout:
      - 'hdf-bundle'         (default): find the largest .hdf, write
        hardfile2= for Amiberry / hard_drive_0= for FS-UAE.
      - 'filesystem-bundle': find an Amiga FS root (C/S/Libs/Devs), write
        filesystem2=/uaehf0=dir for Amiberry / hard_drive_0=<dir> for
        FS-UAE.
      - anything else: no config emitted.

    Returns the path of the generated `.uae`, or the `.fs-uae` if there
    was no `uae_template`, or None if nothing was written.
    """
    spec = asset.install
    ...
    uae_template = spec.get("uae_template")
    fsuae_template = spec.get("fsuae_template")
    if not uae_template and not fsuae_template:
        return None
    ...
    if layout == "hdf-bundle":
        ...
        if hdf is None:
            return None
        if uae_template:
            text = render_uae_template_hdf(uae_template, hdf, extracted_root)
            target = _write_emulator_config(asset.id, "uae", text)
        if fsuae_template:
            text = render_fsuae_template_hdf(fsuae_template, hdf, extracted_root)
            fs_target = _write_emulator_config(asset.id, "fs-uae", text)
            target = target or fs_target
        ...
        if uae_template:
            text = render_uae_template_fs(uae_template, fs_root, extracted_root, volume)
            target = _write_emulator_config(asset.id, "uae", text)
        if fsuae_template:
            text = render_fsuae_template_fs(fsuae_template, fs_root, extracted_root, volume)
            fs_target = _write_emulator_config(asset.id, "fs-uae", text)
            target = target or fs_target
    else:
        return None
...
def _write_emulator_config(asset_id: str, ext: str, text: str) -> Path:
    """Write `text` to ~/Amiberry/conf/amicachy-<id>.<ext> and return the path."""
    out = Path.home() / "Amiberry" / "conf" / f"amicachy-{asset_id}.{ext}"
...
def uninstall_asset(asset_id: str) -> None:
    ...
    (conf_dir / f"amicachy-{asset_id}.fs-uae").unlink(missing_ok=True)
